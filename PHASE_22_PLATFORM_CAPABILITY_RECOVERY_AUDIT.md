...
| 2 | **Parcel** | **B** | `dispatch_attempts` is deliberately polymorphic (Phase B0.3) specifically so a future Parcel vertical can reuse it without a schema change (`PROJECT_CURRENT_STATE.md §23`, confirmed by direct migration inspection this session). `FieldWorker`/capability/wallet/RBAC are all reusable. **No `Parcel`/package model, no parcel-specific order/pricing/dispatch behavior exists.** Named "next" in the roadmap since before this mission began; explicitly not started. |
...
| **Dispatch Engine** | MOSTLY — `dispatch_attempts` is genuinely polymorphic (Phase B0.3). `DispatchService::findCandidates()` itself, however, reads `$booking->service->category_id` directly, coupling the *matching logic* (not just the schema) to the Service catalog shape. | `app/Services/DispatchService.php` read directly | **PARTIAL REUSE.** The `dispatch_attempts` ledger and `RankingEngine` are reusable; `findCandidates()`'s eligibility query would need a parameter for "what am I matching" per vertical rather than assuming `Booking->service->category_id`. |
...
| Parcel completion | Build the actual Parcel order/pricing/dispatch module on top of the already-polymorphic `dispatch_attempts` | Order Engine decision | Dispatch (reuse), Wallet/Commission (reuse once Order Engine resolved) | LOW–MEDIUM | Business: real parcel pricing model |
