...
**Phase 21 progress (updated in place, matching this document's own stated discipline — see §"This document is now the source of truth" at the end):** DOC-1, TECH-1, TECH-2, TECH-3, TECH-4, and TECH-7 are now COMPLETE — see their own entries below for scope and commit hashes. TECH-6 is now STARTED (third increment complete, commit `9011ec4`; 27/35 screens migrated) — genuinely NOT complete, see its own entry below for exactly what shipped and what's still open. Current baseline: 739/739 tests passing, 1741 assertions, HEAD at commit `9011ec4` on `main`, none pushed. All other items in this document remain open exactly as originally assessed.
...
| TECH-6 | **P1** | Admin UI has no design system — 🟡 STARTED, third increment complete (`9011ec4`), genuinely not finished — 27/35 real Livewire screens migrated | Technical | **Yes** (large) |
...
- **Status:** 🟡 **STARTED — third increment complete, see commit `9011ec4`.** Genuinely NOT complete — this item was always scoped as "no single completion bar for a multi-week UI rebuild," and this pass shipped a third bounded, tested increment, not the whole thing.
...
- **What shipped (2026-08-16, commit `9011ec4`, third increment):**
  ...
  - **What was deliberately NOT done:** the other 8 screens (all mid-to-large, 367-856 lines — see "Recommended action" below); a documented visual style guide/Storybook-equivalent; any change to the underlying CDN-Tailwind-via-`<script>` delivery mechanism in `layouts/admin.blade.php`; any UX/interaction redesign beyond what Cms\Manage's modal conversion itself already provided (every other migrated screen renders identically to before, just through shared markup — no screen gained or lost a feature); `Plans\Manage`'s and `PerformanceCampaigns\Manage`'s nested per-row detail tables (entitlements, campaign participants) were deliberately left as plain HTML, matching `Geography\Manage`'s own established nested-table precedent from the second increment — only their row-action buttons were converted to `x-ui.button`; `Roles\Manage`'s one-off blue "Create" staff-account button was deliberately left as plain HTML rather than forcing a new button variant onto a single call site (matches the restraint already shown for `Geography\Manage`'s nested table).
- **Priority:** P1
- **Exact problem (historical, now partially addressed):** Only 7 one-off Blade components existed (confirmed pre-Phase-21: `address-map`, `catalog-tabs`, `icon`, `import-panel`, `setting-override-badge`, `yes-no`, `zone-map`) — no shared button/card/table/badge/modal/status-pill primitives. Every one of the admin panel's real Livewire screens was styled independently. **5 shared primitives now exist (two, `x-ui.modal` and `x-ui.badge`'s solid `emerald`, now proven in real screens, not just isolated tests); 27/35 screens use them; 8/35 still don't** (see the exact list above).
- **Current implementation status:** Third increment shipped and tested. This is still the single largest partially-unstarted piece of work in the entire project — 8 of 35 screens remain to migrate, and no future phase should describe this item as "done."
- **Affected modules/files:** `resources/views/components/ui/*` (button and badge both extended additively), the 27 migrated screens (7 from the first increment + 10 from the second + 10 from the third, listed above), the 8 unmigrated screens listed above, `app/Livewire/Payouts/Manage.php` (new `cancelMarkPaid()` method, second increment, unchanged this pass).
- **Dependency:** None technical — a genuine, multi-week engineering effort, not a decision.
- **Recommended action:** Continue migrating screens in further bounded increments. Only the largest/most complex screens remain: `Settings\Manage` at 856 lines, `Banners\Manage` at 673, `Services\Manage` at 618, `Franchises\Manage` at 576, `NotificationCenter\Manage` at 487, `Zones\Manage` at 388, `Subcategories\Manage` at 374, `Categories\Manage` at 367 — the next increment should expect a slower per-screen pace than this pass's smaller/mid-size batch, and should read each screen fully before committing to a conversion approach (per this document's own scope guardrail: skip and document rather than force a fragile conversion). Per `FINAL_RELEASE_READINESS_AUDIT.md §15`, this can continue to run **in parallel** with API-first mobile app development — it does not block or get blocked by the backend hardening this mission completed.
- **Acceptance criteria:** A real shared component library exists (✅ done, and `x-ui.modal` is now proven in two real screens, not just isolated tests) and at least a meaningful subset of the real Livewire screens are migrated onto it (✅ 27/35 done, 8 remain — this backlog still does not attempt to define a numeric completion bar for the full multi-week UI rebuild; each future increment should keep reporting an honest screen count, not a "complete" claim, until all remaining screens are actually migrated).
- **Code changes required:** **Yes** — large; three bounded increments complete, 8 of 35 screens remain (all mid-to-large).
...
9. **TECH-6** (Admin UI design system) — **third increment shipped 2026-08-16, commit `9011ec4`** (5 shared `x-ui.*` primitives, now including a real `x-ui.modal` wiring in two screens, + 27/35 screens migrated, see its own entry above for exactly what's done and what's not); continue whenever real design/frontend capacity is available. This is the largest single item in the whole backlog and remains its own multi-phase effort — three increments do not close it.
