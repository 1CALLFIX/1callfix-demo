...
**Phase 21 progress (updated in place, matching this document's own stated discipline — see §"This document is now the source of truth" at the end):** DOC-1, TECH-1, TECH-2, TECH-3, TECH-4, and TECH-7 are now COMPLETE — see their own entries below for scope and commit hashes. TECH-6 is now STARTED (second increment complete, commit `31e4f3b`; 17/35 screens migrated) — genuinely NOT complete, see its own entry below for exactly what shipped and what's still open. Current baseline: 726/726 tests passing, 1694 assertions, HEAD at commit `31e4f3b` on `main`, none pushed. All other items in this document remain open exactly as originally assessed.
...
| TECH-6 | **P1** | Admin UI has no design system — 🟡 STARTED, second increment complete (`31e4f3b`), genuinely not finished — 17/35 real Livewire screens migrated | Technical | **Yes** (large) |
...
- **Status:** 🟡 **STARTED — second increment complete, see commit `31e4f3b`.** Genuinely NOT complete — this item was always scoped as "no single completion bar for a multi-week UI rebuild," and this pass shipped a second bounded, tested increment, not the whole thing.
...
- **What shipped (2026-08-16, commit `31e4f3b`, second increment):**
  ...
  - **18 screens remain on the original hand-copied Tailwind classes, unmigrated:** Auth\Login, Badges\Manage, Banners\Manage, Bookings\Index, Bookings\Show, Categories\Manage, Cms\Manage, FlashSales\Manage, Franchises\Manage, NotificationCenter\Manage, Operations\Health, PerformanceCampaigns\Manage, Plans\Manage, Roles\Manage, Services\Manage, Settings\Manage, Subcategories\Manage, Zones\Manage.
  - **What was deliberately NOT done:** the other 18 screens; a documented visual style guide/Storybook-equivalent; any change to the underlying CDN-Tailwind-via-`<script>` delivery mechanism in `layouts/admin.blade.php`; any UX/interaction redesign beyond the Mark Paid cancel affordance noted above (every other migrated screen renders identically to before, just through shared markup — no screen gained or lost a feature); the small nested "cities within a country" inner table on `Geography\Manage` was deliberately left as plain HTML (it's nested inside a `<td>` of the outer `x-ui.table`, where the shared table shell's own card-like chrome wouldn't make sense) — only its row-action buttons were converted to `x-ui.button`.
- **Priority:** P1
- **Exact problem (historical, now partially addressed):** Only 7 one-off Blade components existed (confirmed pre-Phase-21: `address-map`, `catalog-tabs`, `icon`, `import-panel`, `setting-override-badge`, `yes-no`, `zone-map`) — no shared button/card/table/badge/modal/status-pill primitives. Every one of the admin panel's real Livewire screens was styled independently. **5 shared primitives now exist (one, `x-ui.modal`, now proven in a real screen, not just isolated tests); 17/35 screens use them; 18/35 still don't** (see the exact list above).
- **Current implementation status:** Second increment shipped and tested. This is still the single largest partially-unstarted piece of work in the entire project — 18 of 35 screens remain to migrate, and no future phase should describe this item as "done."
- **Affected modules/files:** `resources/views/components/ui/*` (button extended additively), the 17 migrated screens (7 from the first increment + 10 from the second, listed above), the 18 unmigrated screens listed above, `app/Livewire/Payouts/Manage.php` (new `cancelMarkPaid()` method).
- **Dependency:** None technical — a genuine, multi-week engineering effort, not a decision.
- **Recommended action:** Continue migrating screens in further bounded increments (8-10 at a time, as this pass did), saving the largest/most complex screens (`Settings\Manage` at 856 lines, `Banners\Manage` at 673, `Services\Manage` at 618, `Franchises\Manage` at 576) for once the component library has proven itself across more of the simpler/medium screens (`NotificationCenter\Manage` at 487, `Zones\Manage` at 388, `Subcategories\Manage` at 374, `Categories\Manage` at 367 are the next natural size tier). Per `FINAL_RELEASE_READINESS_AUDIT.md §15`, this can continue to run **in parallel** with API-first mobile app development — it does not block or get blocked by the backend hardening this mission completed.
- **Acceptance criteria:** A real shared component library exists (✅ done, and `x-ui.modal` is now proven in a real screen, not just isolated tests) and at least a meaningful subset of the real Livewire screens are migrated onto it (✅ 17/35 done, 18 remain — this backlog still does not attempt to define a numeric completion bar for the full multi-week UI rebuild; each future increment should keep reporting an honest screen count, not a "complete" claim, until all remaining screens are actually migrated).
- **Code changes required:** **Yes** — large; two bounded increments complete, roughly half the screen count remains.
...
9. **TECH-6** (Admin UI design system) — **second increment shipped 2026-08-16, commit `31e4f3b`** (5 shared `x-ui.*` primitives, now including a real `x-ui.modal` wiring, + 17/35 screens migrated, see its own entry above for exactly what's done and what's not); continue whenever real design/frontend capacity is available. This is the largest single item in the whole backlog and remains its own multi-phase effort — two increments do not close it.
