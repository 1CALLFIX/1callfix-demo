...
### BD-8 — Real SMS/push provider ⚠️ P0 — technical preparation COMPLETE (2026-08-16); vendor decision + real credentials still OPEN
- **Priority:** **P0** — still the single highest-priority open item in this backlog; only the *engineering-completable* half is now done.
- **Exact problem (historical, now partially addressed):** `LogSmsAdapter`/`LogPushAdapter` were the only bound implementations. OTP codes and push payloads were written to the server log only — **no real user could receive a login or booking OTP by SMS.**
- **Current implementation status (2026-08-16, commit `1348057`):** Real, tested `SmsAdapter`/`PushAdapter` implementations now exist (`Msg91SmsAdapter`, `GatewayApiSmsAdapter`, `FirebaseFcmPushAdapter`) and `AppServiceProvider` resolves the binding from `config('services.sms.driver')`/`config('services.push.driver')` instead of a static class — but **neither driver is set** (`SMS_DRIVER`/`PUSH_DRIVER` both default to `log`, unset in `.env.example`), so actual delivery is **still dev/QA-only, byte-for-byte unchanged from before this pass**, until an operator deliberately sets one + supplies real credentials. This is deliberate: neither MSG91 nor GatewayAPI was auto-selected (the mission explicitly forbade auto-selecting either), and no credential was fabricated for FCM either.
...
- **Dependency:** None technical remaining for the adapters themselves — purely vendor selection (MSG91 vs. GatewayAPI) + procurement (real credentials for whichever is chosen, plus a real Firebase project/service-account for FCM).
- **Recommended action:** Choose a real SMS vendor (MSG91 or GatewayAPI — both adapters are ready; this backlog does not recommend one) and obtain credentials; obtain real Firebase service-account credentials for the existing/a real Firebase project. Set `SMS_DRIVER`/`PUSH_DRIVER` + the corresponding credential env vars (see `.env.example`) in a real environment. **Real historical vendor-relationship precedent still exists** (Phase 13 evidence, refined above): 1CallFix already had a real Firebase project (`onecallfix-6b538`, credentials expired) and a real MSG91 relationship — real precedent for a vendor RELATIONSHIP, not proof of which SMS vendor should be picked for delivery going forward.
...
- **Code changes required:** **Done** for the adapter/config architecture itself (this pass, no further code needed regardless of which vendor is chosen). **None remaining** once a vendor is chosen — activation is a config/credential change (`SMS_DRIVER`/`PUSH_DRIVER` + the relevant `.env` values), not a new code change. This remains the one BD item this backlog explicitly flags as **launch-blocking**: without a real vendor bound with real credentials, no real customer can complete phone-based login or booking OTP at all.
...
1. **BD-8 (SMS/push provider)** — without a real vendor bound with real credentials, no real customer can complete phone OTP login or booking OTP. This is not a "should," it's a "can't function" gap today. **Technical preparation COMPLETE, 2026-08-16, commit `1348057`** (real `Msg91SmsAdapter`/`GatewayApiSmsAdapter`/`FirebaseFcmPushAdapter`, config-driven binding, both still defaulting to `log` — see BD-8's own entry above and `KNOWN_RISKS_AND_DECISIONS.md` item 8 for the full Provider Responsibility Map). **Still open**: the vendor decision (MSG91 vs. GatewayAPI) and real credentials for whichever is chosen plus a real Firebase project for FCM — none of which this pass could supply.
...
3. **BD-8** (SMS/push vendor decision + integration) — the one true hard launch-blocker. **Technical preparation is now COMPLETE** (commit `1348057`) — real adapters for both SMS candidates plus FCM push exist, config-driven, no further code needed. What remains is purely the vendor decision (MSG91 vs. GatewayAPI — this backlog does not narrow it further than "both are equally ready") and real credential procurement for whichever is chosen, plus a real Firebase project/service-account for FCM.
