...
**Second post-Phase-19 addition (Phase 22.1, 2026-08-16, commit `2bd2257`) — also not part of the 33-screen count:** a new `Modules\Manage` screen (`admin.modules.index`, `/admin/modules`) was added, gated on a new `modules.manage` permission (super_admin-only by default). Lets an operator pick a Country/City/Zone/Franchise and see/toggle each of the 9 registered modules' activation — but only an `is_implemented=true` module (today: `service` only) can ever actually be toggled; every other row renders "Not available" with no click target, both in the UI and re-enforced server-side in `toggle()`. Own dedicated test file `ModulesScreenAuthorizationTest` (6 tests). See `PHASE_22_1_MODULE_ACTIVATION_FOUNDATION.md` for the full mechanism this screen exposes.
...
| **Performance / scale** | Hot-column indexes (9 across 8 tables), N+1 fix, bulk-notification queueing | [IMPLEMENTED] [VERIFIED] — Phase 18; audience-chunking in `CampaignService` resolved Phase 21 item TECH-7, commit `df4e186` (item 28) |
...
**Post-Phase-19 update, 2026-08-16 (Phase 21 items TECH-1/TECH-2/TECH-3/TECH-4/TECH-7):** four of the above are now resolved with a real fix and real verification trail, the same standard item 25 was held to: 22 (commit `cfb1fa6`), 26 (commit `4c1db7c`), 27 (commit `ba4f72e` — resolved for the franchise-scoped operational screens it names; 4 additional scope-shaped timestamps found during that pass remain open, no policy decided), 28 (commit `df4e186`). Item 15's bounded half (read-only viewing) is also resolved (commit `e431667`); what's left of it is no longer a bounded engineering follow-up but a genuine business decision (whether admin intervention/moderation is wanted at all), so it now sits with this section's business-decision items rather than here. 16 and 19 are unchanged, still open. **New finding, item 29** (not in this document's original inventory, discovered 2026-08-16 as a side effect of BD-12's own read-only verification): `FlashSaleService::priceFor()`/`::redeem()` are never called from the real booking-creation path, so Flash Sale discounts currently have no effect on any real booking — blocked on item 12's own stacking decision before it can be wired.
