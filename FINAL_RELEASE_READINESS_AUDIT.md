...
- **`APP_DEBUG` in `.env.example`** — still `true`. This is a *template* default, not evidence of production's real value either way — see §4/risk register item 25. [CHECKED — unresolved, unchanged from baseline]
...
- **`APP_DEBUG` production value** — genuinely unverified, still. `.env.example` ships `true`; nothing in this repository (or any prior session's notes) confirms production's real `.env` was ever corrected to `false`. If it is still `true`, every unhandled exception on the live site renders a full stack trace — file paths, query bindings, potentially environment variable values — to any visitor who triggers one. This is risk register item 25, and it remains the single highest-severity open item in the entire register **specifically because it is also the cheapest to resolve** — verifying it requires one read-only SSH command this mission has never had authorization to run, not a code change.
...
- Production `APP_DEBUG` value unverified (item 25) — carried at baseline in spirit (baseline's `.env.example` note) but only formally logged as a named, numbered risk-register item in Phase 16.
...
**28 items** in `KNOWN_RISKS_AND_DECISIONS.md` as of this phase (baseline predates this file's existence entirely — it was created mid-segment). See `FINAL_ADMIN_CAPABILITY_MATRIX.md` §C for the disposition breakdown (18 genuine business decisions requiring a human call; 7 real closeable engineering follow-ups; 1 confirmed unreachable given current code; 1 verification gap this session had no access to close; 1 historical vendor-selection evidence note). Nothing in the register was silently resolved or invented shut this segment — every item that got new evidence (Phase 13's Glover/6amMart/1.8.10 comparison) had that evidence added as a note, never used to flip a business-decision item to resolved by inference.
...
1. **Verify production's real `APP_DEBUG` value** (risk item 25). This is a five-minute SSH check, not a project. Do this first, independent of every other item in this document — if it's `true`, treat it as a live incident, not a backlog item.
