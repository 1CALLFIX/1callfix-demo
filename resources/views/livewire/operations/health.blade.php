        ...
        <tbody>
            @forelse ($activityLogs as $log)
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2">
                        {{ $log->description }}
                        {{-- REF 1CF-PROMPT-20260925-QA3 — old → new (settings) or the reason (money / referral actions), not just buried in `properties`. --}}
                        @if ($detail = $log->auditDetail())
                            <span class="block font-mono text-xs text-gray-500">{{ $detail }}</span>
                        @endif
                    </td>
                </tr>
            @empty
