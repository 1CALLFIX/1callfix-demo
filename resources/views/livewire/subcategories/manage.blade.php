<div>
    ...
    <x-catalog-tabs current="subcategories" />
    ...
    @if ($showImport)
        <x-import-panel label="Subcategories"
            ...
            validate-method="validateSubcategoriesImport"
            commit-method="commitSubcategoriesImport"
    ...
    <x-ui.card class="mb-6">
        ...
        <div class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex-1 min-w-48">
                ...
                <input type="text" wire:model="name" placeholder="e.g. Split AC, Fan Installation" class="w-full border rounded px-3 py-2 text-sm">
            </div>
            <div class="w-56">
    ...
    @if ($showFilters)
        <div class="bg-white rounded-lg shadow-sm p-3 mb-3 flex items-end gap-3 flex-wrap">
            ...
            @if ($filterModule !== '' || $filterCategory !== '' || $filterActive !== '' || $search !== '')
                <button type="button" wire:click="$set('filterModule', ''); $set('filterCategory', ''); $set('filterActive', ''); $set('search', '')"
        ...
        <thead class="bg-gray-50 text-left text-gray-500">
            ...
            <tbody>
                @forelse ($subcategories as $i => $sub)
                    <tr class="border-t hover:bg-gray-50" wire:key="sub-{{ $sub->id }}">
                        <td class="px-4 py-2 text-gray-400">{{ $subcategories->firstItem() + $i }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ $sub->id }}</td>
                        <td class="px-4 py-2">
                            <span class="w-9 h-9 rounded border flex items-center justify-center overflow-hidden bg-gray-50">
                                @if ($sub->image_url)
                                    <img src="{{ $sub->image_url }}" alt="" class="w-full h-full object-contain">
                                @else
                                    <span class="text-base">{{ $sub->icon }}</span>
                                @endif
                            </span>
                        </td>
                        <td class="px-4 py-2 font-medium">{{ $sub->name }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ $sub->category?->name ?? '—' }}</td>
                        <td class="px-4 py-2 text-gray-400">{{ \App\Support\Modules::label($sub->category?->module) }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ $sub->services_count }}</td>
                        <td class="px-4 py-2">
                            @if ($sub->is_active)
                        ...
                        <td class="px-4 py-2 text-gray-500">{{ $sub->created_at?->format('d M Y') }}</td>
                        <td class="px-4 py-2">
                            <div class="flex items-center gap-1.5 justify-end">
                                ...
                                <button type="button" wire:click="toggleSubcategory({{ $sub->id }})"
                                        wire:loading.attr="disabled" wire:target="toggleSubcategory({{ $sub->id }})"
                                        @class([
                                            'w-8 h-8 rounded flex items-center justify-center text-white',
                                            'bg-red-500 hover:bg-red-600' => $sub->is_active,
                                            'bg-green-500 hover:bg-green-600' => ! $sub->is_active,
                                        ])
                                ...
                                <button type="button" wire:click="confirmDelete({{ $sub->id }})"
                ...
                @empty
                    <tr><td colspan="10" class="px-4 py-6 text-center text-gray-400">
                        @if ($search !== '' || $filterModule !== '' || $filterCategory !== '' || $filterActive !== '')
                            No subcategories match your search or filters.
                        @else
